...
void loop()
{
    ...
    while (true)
    {
        ...
        if (goingToSleep && !Sleeping)
        {
            // Call the function to go to sleep
            enterEnergySaveMode();
        }
        if (Sleeping && !goingToSleep)
        {
            // Call the function to wake up
            wakeupHandler();
        }

        // Battery logic to update the battery level
        if (batteryLevelChanged && BatteryEnabled && !Sleeping)
        {
#ifdef DEBUG
        ...
        if (*SubMenuConnectionVar[_PreferenceUSB] && connectionChanged && !Sleeping)
        {
        ...
        else if (connectionChanged && !Sleeping)
        {
            tft.fillRect(USB_display_image.x,
                         USB_display_image.y,
                         USB_display_image.width,
